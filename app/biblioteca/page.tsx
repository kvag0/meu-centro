...
const createExcerpt = (markdown: string, length = 150) => {
  const plainText = markdown
    .replace(/^#+\s/gm, "") // Remove cabeçalhos
    ...
    <div className="container mx-auto py-12">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="w-full sm:w-auto">
          {/* FUNCIONALIDADE RESTAURADA: Filtro por categoria */}
          <Select
      ...
      {articles && articles.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articles?.map((article) => (
            // FUNCIONALIDADE RESTAURADA: Link para a página de detalhe
            <Link href={`/biblioteca/${article._id}`} key={article._id}>
              <Card className="h-full flex flex-col hover:shadow-lg transition-shadow duration-300 cursor-pointer">
                ...
                <CardContent className="flex-grow">
                  <CardDescription>
                    {/* FUNCIONALIDADE RESTAURADA: Resumo de texto limpo */}
